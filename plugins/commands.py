...
from info import INDEX_CHANNELS, ADMINS, IS_VERIFY, VERIFY_TUTORIAL, VERIFY_EXPIRE, TUTORIAL, SHORTLINK_API, SHORTLINK_URL, AUTH_CHANNEL, DELETE_TIME, SUPPORT_LINK, UPDATES_LINK, LOG_CHANNEL, PICS, PROTECT_CONTENT, IS_STREAM, IS_FSUB, PAYMENT_QR, REACTIONS, PM_DELETE_TIME as pm_delete_time
...
async def start(client, message):
    ...
    if len(message.command) == 2 and message.command[1] == "plans":
        btn = [            
            [InlineKeyboardButton("ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴘᴛ 🧾", url='https://t.me/meera12_bot')],
    ...
    if mc.startswith('verify'):
        ...
        await update_verify_status(message.from_user.id, is_verified=True, verified_time=time.time(), expiry_time=expiry_time)
        if verify_status["link"] == "":
            ...
            reply_markup = InlineKeyboardMarkup(btn)
        await message.reply(f"✅ You Are Successfully Verified For: {get_readable_time(VERIFY_EXPIRE)} Get Files Till Verified Time", reply_markup=reply_markup, protect_content=True)
        return
